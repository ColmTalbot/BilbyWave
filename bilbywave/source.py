...
def chirplet(
        frequency_array, amplitude, q_factor, centre_frequency,
        ellipticity, delta_time, phase, beta):
    """
    ...
    h_plus = (
            amplitude * np.pi**0.5 * tau / 2 / (1 + pi_beta**2)**0.25 *
            np.exp(- np.pi**2 * tau**2 * delta_f**2 / (1 + pi_beta**2) -
                   2j * np.pi * frequency_array * delta_time) *
            (
                    np.exp(1j * (phase + delta - np.pi**3 * beta * tau**2 *
                                 delta_f**2) / (1 + pi_beta**2)) +
                    np.exp(-1j * (phase + delta - np.pi**3 * beta**2 *
                                  delta_f**2) / (1 + pi_beta**2)) *
